...
function iconSvg(input: IdentityInput, monogram: string, variant: IdentityVariant, font: string): string {
...
function wordmarkSvg(input: IdentityInput, variant: IdentityVariant, font: string): string {
...
function lockupSvg(input: IdentityInput, monogram: string, variant: IdentityVariant, font: string): string {
  const height = 120;
  const width = Math.round(Math.max(360, input.name.length * 30 + 180));
...
export function synthesizeIdentity(input: IdentityInput): Identity {
  ...
  const assets: IdentityAsset[] = [
    asset("lockup-main", "Lockup", "lockup", "main", 360, 120, lockupSvg(input, monogram, "main", font)),
    asset("lockup-mono", "Lockup (mono)", "lockup", "mono", 360, 120, lockupSvg(input, monogram, "mono", font)),
