...
- [ ] Run the image-publish workflow and record the resulting staging image evidence before deploying the Container-enabled Worker. The GitHub `staging` environment now has its `CLOUDFLARE_API_TOKEN` secret (configured 2026-07-15); the short-lived token is scoped only to `Workers Containers: Edit` for the `jami-studio` account and expires after 90 days.
- [ ] Provision staging-only `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` Worker secrets through Wrangler's secret interface, then reload authorized, versioned inputs from Supabase inside durable execution. No staging Worker secrets existed when checked on 2026-07-15.
